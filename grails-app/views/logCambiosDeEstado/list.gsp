	...
	<table class="table table-bordered">
		<thead>
			<tr>
		        ...
		        <g:sortableColumn property="observaciones" title="${message(code: 'logCambiosDeEstado.observaciones.label', default: 'Observaciones')}" />
			</tr>
		</thead>
		<tbody>
		<g:each in="${logCambiosDeEstadoInstanceList}" status="i" var="logCambiosDeEstadoInstance">
			<tr class="${(i % 2) == 0 ? 'odd' : 'even'}">
				...
				<td>${fieldValue(bean: logCambiosDeEstadoInstance, field: "modalidad")}</td>
				...
				<td>${fieldValue(bean: logCambiosDeEstadoInstance, field: "observaciones")}</td>
			</tr>
		</g:each>
		</tbody>
	</table>
