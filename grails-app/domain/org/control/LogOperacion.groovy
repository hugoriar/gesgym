package org.control

/**
 * LogCambiosDeEstado
 * A domain class describes the data object and it's mapping to the database
 */
class LogOperacion {

    /* Default (injected) attributes of GORM */
//	Long	id
//	Long	version

    /* Automatic timestamping of GORM */
	Date	     dateCreated
    LogTipoOperacion tipoOperacion
    LogEntidad   entidad
    Long         idEntidad
    Modalidad    modalidad
    UserPersonal personal
    UserSocio    socio
    LogAtributo  atributo
    String       valorAntiguo
    String       valorNuevo
    String       observaciones

//	static belongsTo	= [usuario: UserPersonal]	// tells GORM to cascade commands: e.g., delete this object if the "parent" is deleted.
//	static hasOne		= []	// tells GORM to associate another domain object as an owner in a 1-1 mapping
//	static hasMany		= []	// tells GORM to associate other domain objects for a 1-n or n-m mapping
//	static mappedBy		= []	// specifies which property should be used in a mapping 

    static mapping = {
    }

    static constraints = {
        tipoOperacion nullable: false, blank: false
        entidad       nullable: false, blank: false
        idEntidad     nullable: false, blank: false
        modalidad     nullable: false, blank: false
        personal      nullable: true, blank: true
        socio         nullable: true, blank: true
        atributo      nullable: true, blank: true
        valorAntiguo  nullable: true, blank: true
        valorNuevo    nullable: true, blank: true
        observaciones nullable: true, blank: true
    }

    public ultimoLogOperacion(){
        return findAll().sort{it.dateCreated}?.last()
    }

    /*
     * Methods of the Domain Class
     */
	/*@Override	// Override toString for a nicer / more descriptive UI
	public String toString() {
		return "${dateCreated.toString()} ${socio} - de $estadoAntiguo a $estadoNuevo";
	}*/
}
